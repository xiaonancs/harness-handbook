...
- `Part I Foundations` 处理入口、启动、状态、提示词、主循环、上下文与缓存。
- `Part II Execution` 处理工具、命令、Agent、任务、MCP、权限、配置与 Hooks。
- `Part III Infrastructure` 处理特性开关、API 适配、渲染引擎、设计系统与记忆系统。
...
| [Prompt Cache](Part%20I%20Foundations/07-PromptCache.md) | 缓存边界与成本控制 |
| [Thinking 与推理](Part%20I%20Foundations/08-Thinking与推理.md) | 推理深度的控制面 |

### Part II Execution

| 章节 | 提要 |
|------|------|
| [工具系统](Part%20II%20Execution/09-工具系统.md) | 工具抽象与注册机制 |
| [BashTool](Part%20II%20Execution/10-BashTool.md) | 命令执行与安全约束的交汇点 |
| [命令系统](Part%20II%20Execution/11-命令系统.md) | 命令加载与扩展分发 |
| [Agent 系统](Part%20II%20Execution/12-Agent系统.md) | 子 Agent 的隔离与共享 |
| [内置 Agent](Part%20II%20Execution/13-内置Agent.md) | 角色化 Prompt 的组织方式 |
| [任务系统](Part%20II%20Execution/14-任务系统.md) | 任务执行与前后台切换 |
| [MCP](Part%20II%20Execution/15-MCP.md) | 外部工具接入的协议层 |
| [权限系统](Part%20II%20Execution/16-权限系统.md) | 权限与沙箱的分层判定 |
| [Settings](Part%20II%20Execution/17-Settings.md) | 配置合并与企业策略 |
| [Hooks](Part%20II%20Execution/18-Hooks.md) | 生命周期自动化与执行边界 |

### Part III Infrastructure

| 章节 | 提要 |
|------|------|
| [Feature Flag](Part%20III%20Infrastructure/19-FeatureFlag.md) | 特性开关与构建裁剪 |
| [API 调用](Part%20III%20Infrastructure/20-API调用.md) | 不可靠网络下的请求恢复 |
| [Ink 引擎](Part%20III%20Infrastructure/21-Ink引擎.md) | 终端渲染管线的自定义实现 |
| [设计系统](Part%20III%20Infrastructure/22-设计系统.md) | 终端界面的组件化组织 |
| [Memory](Part%20III%20Infrastructure/23-Memory.md) | 记忆写入与召回路径 |
