# 第 3 章 状态管理 — React 与非 React 世界的状态桥接

> **核心认知**：三层状态架构 — `bootstrap/state` → `Store` + `AppState` → `ToolUseContext` 的极简桥接
...
整个状态管理子系统仅 6 个文件、2,948 行代码（含 1,758 行的 `bootstrap/state.ts`），是 512,664 行代码库中最精简的子系统之一——但它的 API 被 **至少 98 个 React 组件/hooks** 和 **140+ 个非 React 模块** 调用，是整个应用最高频的基础设施。

```mermaid
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#f5f5f5', 'primaryTextColor': '#1a1a1a', 'primaryBorderColor': '#333333', 'lineColor': '#555555', 'secondaryColor': '#e8e8e8', 'tertiaryColor': '#f0f0f0', 'background': '#ffffff', 'mainBkg': '#ffffff', 'nodeBorder': '#333333', 'clusterBkg': '#fafafa', 'clusterBorder': '#999999', 'titleColor': '#1a1a1a', 'edgeLabelBackground': '#ffffff', 'noteTextColor': '#1a1a1a', 'noteBkgColor': '#f5f5f5', 'noteBorderColor': '#999999', 'actorTextColor': '#1a1a1a', 'actorBkg': '#f5f5f5', 'actorBorder': '#333333', 'signalColor': '#333333', 'signalTextColor': '#1a1a1a', 'labelBoxBkgColor': '#f5f5f5', 'labelBoxBorderColor': '#333333', 'labelTextColor': '#1a1a1a', 'loopTextColor': '#1a1a1a', 'activationBorderColor': '#333333', 'activationBkgColor': '#e8e8e8', 'sequenceNumberColor': '#ffffff'}}}%%
graph TD
...
Provider 的 context value 是 `store`（Store 实例本身），而不是 `store.getState()`。Store 实例在创建后引用永远不变。这意味着：
...
**3. `onChange` 在 listener 之前调用**（`L25-L26`）：副作用处理先于 UI 通知。这意味着当 React 组件收到更新信号时，`onChangeAppState` 中的持久化/外部通知已经完成。
...
stateDiagram-v2
    ...
    state CreateSubagent {
        ...
        AC: 默认: 新建子 controller<br/>链接到父级（abort 传播）<br/>opt-in: shareAbortController
        GAS: 默认: 包装 + shouldAvoidPermissionPrompts<br/>shareAbortController: 直接共享
        SAS: 默认: () => {} (no-op)<br/>opt-in: shareSetAppState
        SAFT: 始终穿透到根 Store!<br/>防止 zombie 进程
        RFS: 默认: 克隆副本<br/>防止缓存互相污染
        SETS: 始终新建:<br/>nestedMemoryAttachmentTriggers<br/>loadedNestedMemoryPaths<br/>discoveredSkillNames
        UI: 始终 undefined:<br/>setToolJSX · addNotification<br/>setStreamMode · setSDKStatus
    }
